...
- **`SpaceGrowthProjectionsPanel`** (named export, title "Space Growth Projections"): Space Configuration, Room Utilization Tagging, Enrollment & FTE Projections, Space Growth / Right-Sizing. **⚠️ See "Current uncommitted state" below — this panel is built but not currently wired up anywhere.**
...
### ⚠️ Current uncommitted state — read this before touching `ClassroomUtilizationPanel.jsx` or `StakeholderMap.jsx`
...
| Panel split (`ClassroomUtilizationPanel` / `SpaceGrowthProjectionsPanel`), Room Type auto-suggest, Enrollment & FTE Projections + read-back fix, Space Growth/Right-Sizing + `targetUtilizationRate` fix | — | ⚠️ **Uncommitted.** `SpaceGrowthProjectionsPanel` is currently unwired in `StakeholderMap.jsx` — see flag above |
| `enableClassroomUtilization` | — | `true` locally/uncommitted in `Hastings.json` (Clark actively testing) — do not flip or commit without confirming first |
