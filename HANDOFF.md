...
### Two live StakeholderMap.jsx hotfixes — both confirmed deployed

Later the same day, two unrelated production bugs were found and fixed in `StakeholderMap.jsx` itself (the shared, live-deployed file — not the Classroom Utilization module, which stays behind its disabled flag). Both were pushed individually and confirmed live via the GitHub Actions API (exact-SHA deploy run, `status: completed` / `conclusion: success`) before moving on to other work.

- **`8d3a4a7`** — the cross-tally dedup grouping key (used by both the live free-text Ask Mapfluence payload builder and `classroomScheduleImport.js`'s calc-engine import path) grouped meetings by building+room+day+time+instructor with no session/term field, so two unrelated courses in different blocks that happened to share a room/day/time/instructor were silently merged into one row with a fabricated joined course code (e.g. Farrell-Fleharty 142, Block 1's real `ACCT351` merged with Block 2's real `ACCT110` into a fake `ACCT351/ACCT110` that never existed in either block). Live since `b99cd61` (2026-08-13) — confirmed affecting **29 room+day+time+instructor groups across 18 rooms and 21 instructors** in a full-dataset audit; 22 of the 29 produced a visibly wrong merged course code. Fix: added `sessionRaw` as a seventh grouping-key field in both locations. Verified against the real dataset: 219 → 248 deduped meetings, all 29 previously-merged groups now split correctly by session, zero regressions on legitimate same-block cross-tallies (e.g. `EDUC630/EDUC430`).
- **`0eddea0`** — `normalizeClassScheduleRoomKey()` (shared by the free-text room-key filter, the room-click popup's weekly schedule, and `getRoomScheduleSnapshot`) built its room-number half of the key from Airtable's raw field with no normalization, while the class-schedule/registrar side stores bare numbers. A full 9-building audit against real schedule + Airtable data found **three distinct mismatch shapes**, not one: Farrell-Fleharty bakes an `FC-` prefix into every room number (`FC-142` vs bare `142`, 6/6 rooms affected — this is what surfaced first, via a live "Farrell-Fleharty 142" query returning zero results); Kiewit is a mixed case (`GYM` only matches Airtable's `K GYM`, space-separated; `SPC` has no Airtable counterpart at all and is intentionally left unmatched rather than force-fixed); Scott Studio Theater is a different bug entirely, on the *schedule* side — the registrar workbook stores three inconsistent verbose labels (`"118 - Theater"`, `"118 -Theat"`, `"118 - Theat"`) instead of Airtable's clean bare `118`. Fixed with `CLASS_SCHEDULE_ROOM_PREFIX_STRIP` (mirrors the calc-engine's own prefix-strip convention, see below) plus a leading-room-number extractor for the Scott Studio case. Verified: 37/48 → 47/48 schedule room references now resolve correctly across all 9 buildings; the one remaining gap (Kiewit's `SPC`) confirmed to degrade gracefully, not throw.

### Real registrar scheduling conflict found — Farrell-Fleharty 142 (flag to Hastings, not a code bug)

While diagnosing an apparent Time Utilization bug (Farrell-Fleharty 142 showing 104% for Fall 2026 Block 1 — a room can't legitimately exceed 100% of available hours), traced every meeting in that room/term back to the real `/class-schedule` data and checked for overlapping day/time slots. Found a genuine conflict, not a bug: **four different courses — `ACCT100`, `ACCT351/ACCT110`, `ACCT351`, and `FINA361` — are all assigned to room 142 at essentially the same 10:20–11:40 slot**, taught by three different instructors (Van O'Linda, Carpenter, Saha). Confirmed this isn't a cross-tally dedup miss (the dedup key includes instructor, and these genuinely have different instructors, so they were correctly kept as separate meetings). Working backward from the two live percentages Clark reported (Farrell 142 at 104%, Hurley-McDonald 229 at 80%) independently confirms Fall 2026 Block 1's `standardWeeklyHours` is `40` — both rooms' real hours divided by 40 match exactly. **This is a real source-data conflict for the registrar to resolve, not a code defect** — no physical room can host 4 different courses simultaneously. Worth surfacing to Hastings directly; nothing to fix here in the app.

### Utilization Calc Engine built and committed (`dc27e6d`) — roadmap item 6 of 6, roadmap now complete

New `src/utils/classroomUtilizationCalc.js` + a new `UtilizationResultsSection` in `ClassroomUtilizationPanel.jsx` — the first real output of this module: a per-room, per-term Time/Seat Utilization table, read-only, no writes. **Room+term grain, not room-level**: a room used in both Fall 2026 Block 1 and Block 2 produces two separate rows, each scored only against that term's own meetings and `standardWeeklyHours` — never blended (this was a design call made mid-build, then explicitly confirmed by Clark as the correct approach). A meeting whose `sessionRaw` doesn't resolve to any `terms` doc has no row to belong to at all and is surfaced in a visible "N meetings couldn't be matched to a term" banner rather than being silently dropped or defaulted.

Building the first real Time/Seat Utilization output against live data immediately surfaced three real, previously-invisible data-correctness bugs, all found via anomalous 0% results and confirmed fixed against real Firestore/Airtable data:

1. **`"ALL"` daysText producing empty `dayTokens`** (`classroomScheduleImport.js`). ai-server's `normalizeScheduleDayTokens()` doesn't expand `"ALL"` to actual weekday tokens — it falls through every branch and returns `[]`, identical to genuinely-unscheduled `TBA`/`ARR` rows. Silently made Time Utilization compute a real 0% for meetings that actually run every weekday (found via Gray Center 116, Fall 2026 Block 1: 2 real meetings, 0% shown). Fixed client-side, not in `server.js`, since the raw `daysText` this decision needs was already available at import time.
2. **Null enrollment/capacity stored as literal `0`** (`classroomScheduleImport.js`). `Number(null)` is `0` in JS, so the old ternary treated a genuinely-missing value as "a real number: 0" and wrote `0` to Firestore instead of `''`. Every meeting from a workbook with no enrollment column showed a computed "0%" Seat Utilization instead of "pending enrollment data" (same Gray Center 116 case). Fixed by checking for `null`/`undefined`/`''` before ever calling `Number()` on the value, while still preserving a genuine `0` (a cell that literally contains `"0"`).
3. **Airtable capacity-join mismatches** (`classroomUtilizationCalc.js`). Farrell-Fleharty (`"Farrell Arena Fleharty Educational Center"` in Airtable vs `"Farrell-Fleharty"` in `courseMeetings`) and Scott Studio Theater (`"Theatre"` vs `"Theater"` spelling) never joined on building name; Farrell-Fleharty also bakes the same `FC-` prefix into Airtable's room number that the room-key hotfix above had to work around separately. Every room in these buildings showed "capacity unknown" despite Airtable having real `Seat Count` data. Checked all 10 Hastings buildings with scheduled classrooms against live data — only these two diverge. Fixed with a per-building override map, scoped narrowly so nothing else is touched.

Also polished in the same commit: Room Utilization Tagging is now collapsed by default (`<details>`/`<summary>`, same pattern `CapitalPrioritiesPanel.jsx` already uses — the panel had grown to five stacked sections and this one is a one-time setup task), and the Time Utilization explanation now shows each row's actual entered Standard Weekly Hours inline next to its term instead of a generic static description.

`enableClassroomUtilization` was `false` in this commit, same convention as every prior session. `ai-server/server.js` and `firestore.rules` untouched by this commit. **Correction, added 2026-08-20:** the note directly above originally said `dc27e6d` was local-only/not yet pushed — it has since been pushed (carried along as a fast-forward ancestor of `a9cd6e0`, see the retirement section below) and is now live on `origin/feature/multi-university-refactor`, confirmed deployed.

**Roadmap status: 6 of 6 items complete.** Shell/schema, Space Configuration, Terms, Room Utilization Tagging, and now the Utilization Calc Engine are all done. **Update, 2026-08-19/20: the roadmap didn't stop there** — see the sections below for everything built and shipped since, including a full second panel (Enrollment & FTE Projections, Space Growth/Right-Sizing) and a live-site retirement of the old utilization card it supersedes.

### Real Fall 2026 data cutover, closed out — file-selection heuristic risk eliminated

Following the `courseMeetings` re-import documented above (219 → 248 deduped meetings against the real combined Fall 2026 Block 1+2 workbook, after `5d31af0`/`e6f768f`'s building-alias extension and test-workbook retirement), the remaining risk flagged in the 2026-08-13 guardrails was closed for good: **`CLASS_SCHEDULE_FILE_NAME` is now set explicitly in the Hastings AI server's Render environment**, pointing directly at the real workbook rather than relying on `resolveClassScheduleFilePath`'s filename-scoring heuristic to keep picking the right file as new registrar deliveries land in `ai-server/Docs/` (that folder now also has several real per-block grid files sitting alongside the combined workbook — exactly the kind of ambiguity the heuristic was never reliable against, per the original 2026-08-13 guardrail note). The whole class-schedule pipeline — Ask Mapfluence free-text, the room-click weekly schedule, and the Classroom Utilization Planner's Import Schedule button — now runs against real registrar data end to end, with no scoring ambiguity left for ai-server to resolve at request time.

### Classroom Utilization module completed and split into two panels

The module grew past a single stacked panel and was split into two separate `.dashboard-box` exports from `ClassroomUtilizationPanel.jsx`, both gated by the same `enableClassroomUtilization` flag (no second flag introduced) and mounted side by side in `StakeholderMap.jsx`:

- **`ClassroomUtilizationPanel`** (default export, title "Classroom Utilization"): Import Schedule, Terms, Utilization Results — the Time/Seat Utilization calc engine from `dc27e6d`, room+term grain, never blended across blocks.
- **`SpaceGrowthProjectionsPanel`** (named export, title "Space Growth Projections"): Space Configuration, Room Utilization Tagging, Enrollment & FTE Projections, Space Growth / Right-Sizing. **Resolved 2026-08-20** — was unwired when this was first written; see the closing addendum below, it's now committed, wired up, and live.

Every section in both panels is now collapsible (`<details>`/`<summary>`, the same pattern `RoomUtilizationMetaSection` established), consistent across the whole module — Space Configuration and Terms picked this up in the same pass as the split (previously always-expanded `<h4>` headers).

**Room Utilization Tagging — Airtable Room Type auto-suggest added** (`src/utils/roomTypeSuggestion.js`, new file, per Clark's 2026-08-19 request). Of the 48 rooms `courseMeetings` actually schedules classes in, 37 match an Airtable room record with a non-blank Room Type Description, across exactly 7 distinct values. A hardcoded map (`ROOM_TYPE_TO_SPACE_CATEGORY`) groups those into `Classroom`/`Lab` suggestions (mirrors the "100 Classrooms / 200 Labs" split precedent from the Strategic Dashboard, borrowed as prior art only — no shared data path). Fetched read-only from the same `/ai/api/rooms` endpoint `UtilizationResultsSection` already uses, applied **once per mount** to any room with no existing tagging doc and a still-blank category — never auto-saves, never creates a new `spaceConfig` category on its own (if the suggested category doesn't exist yet in `spaceConfig`, the suggestion is silently skipped rather than force-created). Suggested-but-unconfirmed rows get a distinct blue "suggested" badge/highlight, separate from the existing amber "dirty/unsaved" styling; any manual edit — even re-picking the same value — downgrades a row out of "suggested" into a normal manual edit. The 11 rooms that don't resolve to any Airtable room (Farrell-Fleharty ×7, Kiewit GYM/SPC, Scott Studio Theater ×2 — the same three room-key mismatch shapes `0eddea0` fixed for the schedule side) simply have nothing to suggest from and fall back to the existing manual dropdown, unchanged.

**Enrollment & FTE Projections built**, including a real UX fix caught and closed out this session. The upload/preview/Confirm & Save flow (`src/utils/enrollmentProjectionsImport.js`, new file — parses structurally by label text and year-header position, not fixed offsets, verified against the real `ai-server/Docs/Hastings enroll_FTE.xlsx`) was working, but `EnrollmentProjectionsSection` never read back what was actually saved — after a successful save it showed only a transient local text count ("13 records saved") that reverted to looking unsaved on the next page load or reopen, even though the data was sitting in Firestore the whole time. **Fixed:** the section now does a `getDocs`-on-mount load (`loadSavedProjections`, same pattern `SpaceConfigSection`/`TermsSection` already use) plus a reload after every successful save, and renders a read-only "Currently saved (N records)" table — division, department, latest-year headcount and total FTE — below the upload control. The summary label now reflects actual persisted Firestore state immediately on load/reopen, not just transiently within the save session. Real departmental data has been uploaded and verified this way: **13 records** (12 departments + 1 institution-wide "Overall" record, confirmed against the real workbook's structure — exactly 12 department names resolve, zero stray records from the workbook's "Offsets" sub-block since it matches no known metric label).

**Space Growth / Right-Sizing built** (`src/utils/spaceGrowthCalc.js`, new file) — the first real consumer of `enrollmentProjections`, and the roadmap item beyond the original 6 flagged as a future step back on 2026-08-13. Per space category: Current SF (Airtable area summed across rooms tagged with that category in Room Utilization Tagging), Ideal SF now and at an admin-selectable target year (category's Ideal NSF/Student × institution-wide enrollment for that year), and both gaps. Per Clark's explicit decision, this first version uses **institution-wide enrollment only**, deliberately ignoring the 12 per-department `enrollmentProjections` records even though they exist in the same collection and only reading the single "Overall" record — flagged prominently in the section's own UI text so it's never mistaken for a more precise per-program breakdown. See the bug writeup immediately below for a real correctness issue caught in this formula before it was ever exercised against real numbers.

### Real bug found and fixed: `targetUtilizationRate` omitted from the Space Growth Ideal NSF/Student formula

**The bug:** an earlier version of `spaceGrowthCalc.js` computed "Ideal NSF/Student" as a direct pass-through of `spaceConfig`'s `sfPerStationTarget` field alone. That's wrong per the original spec — a category's SF/station target describes the space *at full utilization*, but real-world scheduling never hits 100%, so the space actually needs to be *provisioned* larger than the raw target to still achieve that target utilization rate once realistic slack is accounted for. **Fix:** `idealNsfPerStudent = sfPerStationTarget / targetUtilizationRate` (both already-existing `spaceConfig` fields — no new field added, Space Configuration itself untouched). E.g. a 25 SF/station target at a 70% target utilization rate means the space needs to be provisioned at 25/0.7 ≈ 35.71 SF/student to still hit that utilization in practice — the pass-through-only version silently understated Ideal SF, and therefore understated every category's SF gap, by exactly the utilization factor (a ~30% understatement at a typical 70% target rate). A zero/missing/non-positive `targetUtilizationRate` is treated as "not set" (division skipped, `idealNsfPerStudent` is `null`, same convention as a missing `sfPerStationTarget`) — never divide by zero, never fabricate a derived value from an incomplete input. **Caught in review before this module was ever exercised against real numbers** — not found via a live anomalous result like the three Utilization Calc Engine bugs documented above, but the same root-cause standard applies: this is a real formula-correctness bug, not a cosmetic one, and would have silently understated every category's space gap in the live tool had it shipped.

### Registrar data-quality items relayed to Hastings by email

Three findings from this module's build-out were written up and sent to Hastings directly, since none of them are app bugs — they're either genuine source-data conflicts or open questions about registrar data conventions:

1. **Farrell-Fleharty 142 real scheduling conflict** (documented in full above, under "Real registrar scheduling conflict found") — four different courses (`ACCT100`, `ACCT351/ACCT110`, `ACCT351`, `FINA361`), three different instructors, all assigned to the same room at essentially the same 10:20–11:40 slot in Fall 2026 Block 1. No physical room can host four simultaneous courses; this needs registrar resolution, not a code fix.
2. **What does an `"ALL"` day-pattern mean in the registrar's convention?** Tied to the `"ALL"` daysText bug fixed in the Utilization Calc Engine section above (ai-server's day-token normalizer didn't expand `"ALL"` to real weekday tokens, silently producing `0%` Time Utilization for meetings that actually run every weekday) — the code-level fix assumes `"ALL"` means every weekday (Mon–Fri), but that's an inference, not a confirmed registrar convention. Asked Hastings directly to confirm.
3. **Scott Studio Theater's inconsistent room labels** — the registrar workbook stores three different verbose spellings for the same room (`"118 - Theater"`, `"118 -Theat"`, `"118 - Theat"`) instead of Airtable's clean bare `118`. The `0eddea0` hotfix already works around this on the app side (a leading-room-number extractor), but flagged to Hastings as a data-hygiene item worth cleaning up at the source, since it's the kind of inconsistency that can resurface in some other form later.

### Retirement: old CSV-backed "Classroom Utilization" card removed from Campus Summary (`a9cd6e0`, `9f68cb4`)

The pre-existing, **client-facing** (shown to both Hastings and Cherokee, gated `!isSarpyCountyInstance`) "Classroom Utilization" card in `SpaceDashboardPanel.jsx` — static-CSV-backed Time/Seat Utilization bars (`UtilizationBars`) plus a "Classroom Utilization Heat Map" checkbox — was retired outright. Trigger: its scrollbar was clipping displayed percentages (e.g. "34.9%" showing cut off); since the card was superseded by the now-complete Classroom Utilization Planner module above (real schedule data, calc engine, growth/right-sizing, all verified against live data this session), it was removed rather than patched. **`Strategic Space Dashboard`, the separate collapsible subsection directly below it, was explicitly left untouched** — confirmed still working after the change.

Before removing, checked whether the underlying static-CSV pipeline (`public/Data/Utilization/classroom_utilization.csv`, `UTILIZATION_CSV_PATH`, `parseUtilizationCsv`, `getUtilizationForBuilding`, `utilizationData`/`utilizationByBuilding`/`utilizationByRoom`/`utilizationCampus` state in `StakeholderMap.jsx`) was dead code. **It is not** — it's still load-bearing for two other, unrelated live features: the building-click popup's `UtilizationBars` card (`panels/BuildingPanel.jsx`, a completely separate per-building display from the retired dashboard card) and Strategic Space Dashboard's own enrollment-baseline calc (`utilizationCampus?.finalEnrollment`). Neither was touched. Only the now-dead caller-side wiring that existed solely to feed the deleted card — `dashboardUtilization`/`dashboardUtilizationLabel` (`useMemo`s), `showClassroomUtilizationDashboard`, and the five now-nonexistent props being passed into `<SpaceDashboardPanel>` — was removed from `StakeholderMap.jsx`. `utilizationHeatmapOn` state was deliberately **left in place as inert dead code** rather than surgically extracted: its only setter call was the deleted checkbox (so it can now never become `true` again), but it's read inside one large, ~130-line shared building-recolor effect that also drives technical-assessment coloring and maintenance-workflow coloring — untangling just the heatmap branch safely needed more review than this pass warranted.

**Process failure and fix, worth remembering for future commits on this branch:** the first commit (`a9cd6e0`) broke the GitHub Pages build. `git add`-ing the whole `StakeholderMap.jsx` file staged its *entire* current content, not just the retirement-scoped edits — it also swept in an unrelated, pre-existing uncommitted change (an import + render of `SpaceGrowthProjectionsPanel`, left over from the Enrollment/Space Growth work above) that referenced an export only present in the deliberately-excluded, still-uncommitted `ClassroomUtilizationPanel.jsx`. The committed tree didn't build standalone even though the working directory did — masked by the many other uncommitted files still sitting in the tree. **Caught via the Actions API** (`build` job step failed at `vite build`; `deploy` job never ran at all, `conclusion: skipped`, confirmed via the run's job/step breakdown — so the old card was never actually taken off the live site, GitHub Pages kept serving the last successful deploy, `dc27e6d`, the whole time). Fixed forward with `9f68cb4`, reverting just the stray `SpaceGrowthProjectionsPanel` import/render block. **Before pushing the fix, verified it properly this time**: extracted the exact commit tree in isolation via `git archive <sha> | tar -x` into a scratch directory and ran `npm run build` there, rather than trusting the working-directory build. `9f68cb4`'s deploy run confirmed `completed`/`success` via the Actions API. **Lesson for next session:** given how much parallel uncommitted work sits in this tree at any time, verify a commit's tree builds in isolation before pushing — a diff review alone can miss a cross-file break like this one.

### ⚠️ Current uncommitted state (RESOLVED 2026-08-20 — see closing addendum below; kept here for the historical record of what shipped and how)

- **`ClassroomUtilizationPanel.jsx` still has substantial uncommitted work**: the panel split itself, `RoomUtilizationMetaSection`'s Airtable auto-suggest, `EnrollmentProjectionsSection` (including the read-back fix above), `SpaceGrowthSection`, and the `SpaceGrowthProjectionsPanel` export — none of it committed yet. New, also-uncommitted helper files it depends on: `src/utils/enrollmentProjectionsImport.js`, `src/utils/roomTypeSuggestion.js`, `src/utils/spaceGrowthCalc.js`. `src/utils/classroomUtilizationSchema.js` (adds `ENROLLMENT_PROJECTIONS_COLLECTION`) and `firestore.rules` (adds the matching `enrollmentProjections` match block, same `isUniversityAdmin` posture as every sibling collection) are uncommitted too.
- **`SpaceGrowthProjectionsPanel` is fully built but NOT reachable anywhere right now.** As part of the `9f68cb4` build fix above, `StakeholderMap.jsx`'s import of `SpaceGrowthProjectionsPanel` from `ClassroomUtilizationPanel.jsx` and its `<div className="dashboard-box"><SpaceGrowthProjectionsPanel .../></div>` render block were both removed — that wiring depended on an export that only exists in the still-uncommitted `ClassroomUtilizationPanel.jsx`, and shipping it broke the live build. The panel component itself still exists, uncommitted, in `ClassroomUtilizationPanel.jsx` — it's just not imported or rendered by anything anymore, even locally. **Next session (or whoever commits this work) needs to re-add that import + render block to `StakeholderMap.jsx` in the same commit as `ClassroomUtilizationPanel.jsx`'s other changes** — committing one without the other reproduces the exact `a9cd6e0` build break.
- **`src/Configs/Hastings.json` has `enableClassroomUtilization: true`, uncommitted.** This is intentionally left on — Clark is continuing to work with the module live for local testing, not ending the session — unlike the usual end-of-session convention (documented earlier in this file) of flipping it back to `false` before wrapping up. **Do not flip this back to `false` or commit it as part of an unrelated change** without explicit confirmation the session is actually ending.

### Known follow-up, not yet started

Clark flagged, as a future want and not yet scoped: replacing the building-click popup's `UtilizationBars` card (`panels/BuildingPanel.jsx`, still backed by the static `classroom_utilization.csv` pipeline confirmed load-bearing above) with real data from the now-complete Utilization Calc Engine (`classroomUtilizationCalc.js`/`courseMeetings`/`terms`), so the per-building popup and the Classroom Utilization Planner's own Utilization Results table are showing the same real-schedule-derived numbers instead of two different data sources. No design or scoping work has started on this — noted here so it isn't lost.

### Current status (end of session, 2026-08-20)

| Item | Commit | Status |
|---|---|---|
| `sessionRaw` cross-tally grouping fix (Block 1/2 false merges) | `8d3a4a7` | ✅ Live, deployed |
| Room-key normalization (Farrell-Fleharty/Kiewit/Scott Studio) | `0eddea0` | ✅ Live, deployed |
| Utilization Calc Engine + 3 data-correctness fixes (roadmap 6/6) | `dc27e6d` | ✅ Pushed and deployed (was local-only when first written above — corrected) |
| `CLASS_SCHEDULE_FILE_NAME` pinned on Render | — (Render env config, not a commit) | ✅ Done — confirmed by Clark |
| Old CSV-backed Classroom Utilization card retirement | `a9cd6e0` (build broke), `9f68cb4` (fix) | ✅ `9f68cb4` deployed and confirmed live; `a9cd6e0` alone never went live (build failed before the deploy job ran) |
| Panel split (`ClassroomUtilizationPanel` / `SpaceGrowthProjectionsPanel`), Room Type auto-suggest, Enrollment & FTE Projections + read-back fix, Space Growth/Right-Sizing + `targetUtilizationRate` fix | `dd9f95e` | ✅ Committed, pushed, deployed — `SpaceGrowthProjectionsPanel` re-wired into `StakeholderMap.jsx` in the same commit, isolated-build-verified before push |
| `enableClassroomUtilization` | `d4d3e03` | ✅ `true`, committed, pushed, deployed — both panels now live for C&E admin users (see closing addendum below) |
| Registrar data-quality items (Farrell 142 conflict, `"ALL"` day-pattern question, Scott Studio labels) | — | ✅ Emailed to Hastings |
| Hastings-popup `UtilizationBars` → calc-engine data | — | ⚠️ Not started, future want only |
| Hastings / Sarpy / Cherokee | — | ✅ Confirmed unaffected — Sarpy/Cherokee untouched by any change this session; retirement change confirmed working live by Clark on both Hastings (card gone, Strategic Space Dashboard intact, building popup still works) and Cherokee (unaffected) |

### Addendum: module fully live — nothing left uncommitted or unpushed

`enableClassroomUtilization` flipped `false` → `true` in the committed `Hastings.json` and pushed (`d4d3e03`). Confirmed safe by Clark: no Hastings staff hold admin credentials to this site — only C&E's team does — so this only affects internal/admin visibility, zero client-facing exposure (both panels are gated behind `isAdminMode`, per the retirement section above). Both panels — Classroom Utilization and Space Growth Projections — visually confirmed working on the live site by Clark. **This closes out the "uncommitted state" flag earlier in this entry** — nothing from today's Classroom Utilization work remains uncommitted or unpushed.

### Continued, same day: department × category growth breakdown (`66973cf`)

The Space Growth / Right-Sizing table above was, until this point, institution-wide only — per Clark's original 2026-08-19 decision, deliberately ignoring the 12 real per-department `enrollmentProjections` records in favor of the single "Overall" one, flagged prominently in the UI so it was never mistaken for more precise than it was. Clark asked for the full-fidelity version next: a `primaryDepartment` field added to `roomUtilizationMeta` docs (optional, not backfilled onto existing tagged rooms), a new `computeDepartmentSpaceGrowth()` in `spaceGrowthCalc.js` that groups the same tagged rooms by (spaceCategory, primaryDepartment) pair and prices each pair's Ideal SF against that specific department's own enrollment instead of the institution-wide total, and a "By Department" table rendered below the existing institution-wide one in `SpaceGrowthSection` — same target-year selector, same fetched Firestore/Airtable snapshot (one fetch, two computations). Purely additive: `computeSpaceGrowth()` and the institution-wide table are untouched. A tagged room with a category but no department assigned is excluded from the new breakdown but still counts normally in the institution-wide table, with the excluded count surfaced in a visible banner. Isolated-build-verified (`git archive` into a clean scratch dir + `npm run build`) before push, same discipline as `9f68cb4`'s build-break lesson above.

Both the "keep it simple" (institution-wide) and "full fidelity" (per-department) versions of the growth model Clark asked for along the way now coexist, not one replacing the other.

### Continued, same day: Airtable-driven primary department auto-suggest (`99f0f73`) — and the debugging journey behind it

With `primaryDepartment` now a real field, the obvious next gap was that Room Utilization Tagging's manual dropdown was the only way to fill it in — 48 rooms, one at a time. Clark asked for the same Airtable-auto-suggest treatment `roomTypeSuggestion.js` already gives `spaceCategory`.

**The crosswalk.** Airtable's Rooms table has a `Department` field, already exposed by `/api/rooms` (confirmed via a live pull — no `server.js`/`.env` change was needed for this). It is **not** a 1:1 match to `enrollmentProjections`' 12 real department names — checked against live data, not assumed, same caution the Room Type mapping needed: only 3 of the 12 (Art, Biology, Languages & Literatures) are exact string matches; the rest are Airtable's finer per-discipline values that need grouping into `enrollmentProjections`' combined budget-line departments (e.g. Physics + Chemistry → "Chemistry & Physics", Psychology → "Psychology & Sociology"). Built as a hardcoded, reviewed map (`src/utils/departmentSuggestion.js`, mirrors `ROOM_TYPE_TO_SPACE_CATEGORY`'s discipline) rather than fuzzy matching. Coverage: **43 of the 48** `courseMeetings`-scheduled rooms get a confident suggestion; the other 5 (Kiewit `GYM`/`SPC`, Scott Studio Theater ×3) have no Airtable record at all under the existing room-key join — the same pre-existing gap Room Type suggestion already has, not a new one.

**Mount-time suggestion**, wired exactly like `spaceCategory`'s: only for rooms with no existing `roomUtilizationMeta` doc, only into a blank field, never auto-saved, own independent "suggested, not yet saved" badge.

**The debugging journey (worth keeping in the record).** After shipping the mount-time suggestion, Clark reported it wasn't appearing for any room. Investigated a suspected race condition first (the effect's gate on `departmentOptionsLoaded`) — traced the actual effect ordering and dependency array, confirmed the gate was correct, ruled it out. Re-verified the crosswalk itself against a fresh live pull of real data (43/48, zero mismatches) — ruled that out too. Added temporary `[DEPT-SUGGEST-DEBUG]` console logging at every stage (listener resolution, effect gate state, per-room trace, the Airtable fetch's start/success/error/settle) rather than continuing to guess, and had Clark check his own browser console against a clean dev-server restart. The logs showed `airtableSuggestionsLoaded` never flipping true — but tracing *that* (same fetch that already powered the working Room Type suggestion, unchanged by this work) turned out to be a dead end too: the real finding, confirmed by Clark, was that **the suggestion effect was working correctly the whole time** — it deliberately skips any room with an existing `roomUtilizationMeta` doc, and all 48 rooms already had one from an earlier manual tagging pass (mostly `spaceCategory` only, saved before `primaryDepartment` existed). There was never a bug in the gate or the fetch; there was simply no backfill mechanism for already-tagged rooms. All debug logging was removed once this was confirmed (verified via `git diff` showing zero `console.log`/`DEBUG` lines before committing).

**Fix: "Suggest Departments" button**, the manual, repeatable equivalent for already-tagged rooms — same crosswalk, same live data, only ever fills a currently-blank `primaryDepartment`, never touches a room that already has one (even if a different suggestion would be available), shows a one-line summary after running (e.g. "Suggested departments for 43 rooms; 5 had no Airtable match"). Same "admin reviews, clicks Save to confirm" convention as everything else in this module — neither path writes directly. Tested and confirmed working end-to-end by Clark: suggestions applied via both paths, saved, and flowing correctly into the "By Department" table above.

**Incidental fix found during the investigation:** the `spaceConfig` and `enrollmentProjections` `onSnapshot` listeners' error handlers set `loadError` but never marked their `*Loaded` flag `true` — meaning a failed read would silently stall the suggestion effect forever (both fields, since they share one gate), with no visible failure beyond the easy-to-miss `loadError` text. Both now mark `*Loaded` true on error too, so a failed read degrades to "loaded with zero options" instead of hanging indefinitely.

### Status: Classroom Utilization module now feature-complete, including per-department growth precision

All 6 original roadmap items, the panel split, the retirement of the old CSV-backed card, Room Type auto-suggest, Enrollment & FTE Projections with a working read-back, both the institution-wide and per-department Space Growth models, and department auto-suggest (mount-time + backfill) are all built, committed, pushed, and confirmed live/working by Clark. `enableClassroomUtilization` remains `true` — see the addendum above for why that's intentional and confirmed safe, not something to revert.

**Remaining known follow-up, not yet started:** replacing the building-click popup's still-CSV-backed `UtilizationBars` (`panels/BuildingPanel.jsx`) with real data from the now-complete Utilization Calc Engine, so the popup and the Planner's own results table show numbers from the same source instead of two different ones — flagged above, unchanged, not yet scoped.

**⚠️ `functions/index.js` / `functions/package.json` — still uncommitted, still unreviewed, flagging again so this doesn't get lost.** A Cloud Functions v1→v2 API migration (Node 18→22, `firebase-admin`/`firebase-functions` version bumps, `onCall`/`onRequest` from `firebase-functions/v2/https`, explicit `invoker: "public"`) has sat uncommitted in the working tree all session, deliberately excluded from every commit above per explicit instruction — separate, unreviewed work, not yet deployed to Firebase (a `firebase deploy --only functions` is a separate, real-infra-changing step nobody has run). Confirmed via isolated-build checks throughout the session that none of the Classroom Utilization commits depend on it. **Do not commit or deploy this without a deliberate, separate review** — it should not get swept in accidentally by a future broad `git add`.
