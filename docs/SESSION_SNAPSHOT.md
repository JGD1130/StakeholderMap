...
- 2026-04-30: Removed the extra small building summary popup from the full-admin Technical workflow on `/:universityId/admin`. Building clicks in Technical were opening both the Technical Assessment panel and the generic admin building popup; updated the shared building-click handler so Technical opens only the assessment panel. Validation: `npm.cmd run smoke` passed and `npm.cmd run build` passed.
- 2026-04-29: Fixed full-admin Engagement building recolor regression on `/:universityId/admin`. In the shared admin map, switching to `Engagement` was still using the Technical assessment shading branch because the building recolor effect treated admin `ASSESSMENT` as technical mode. Updated `StakeholderMap` so technical building colors apply only when the Technical workflow is active. Validation: `npm.cmd run smoke` passed and `npm.cmd run build` passed.
- 2026-04-29: Retired legacy admin workflow URLs by redirecting `/:universityId/admin/engagement` and `/:universityId/admin/technical` to the full admin map. Moved remaining Engagement-only behavior in `StakeholderMap` onto the shared full-admin `Engagement` workflow flags so marker tools, floor scope, room sentiment, and marker rendering work from `/admin`. Kept public `/:universityId/engagement` and `/:universityId/technical` routes intact. Fixed full-admin Technical so switching map views no longer auto-opens a prior selected building panel and closing the Technical Assessment panel no longer reverts the map view to Engagement. Fixed full-admin Engagement floorplan controls so the Floorplan/Load buttons explicitly load the selected floor. Updated smoke checks and Hastings pre-release checklist to validate the redirects and full-admin Engagement/Technical views. Validation: `npm.cmd run smoke` passed and `npm.cmd run build` passed.
- 2026-04-29: Retired legacy admin workflow URLs by redirecting `/:universityId/admin/engagement` and `/:universityId/admin/technical` to the full admin map. Moved remaining Engagement-only behavior in `StakeholderMap` onto the shared full-admin `Engagement` workflow flags so marker tools, floor scope, room sentiment, and marker rendering work from `/admin`. Kept public `/:universityId/engagement` and `/:universityId/technical` routes intact. Fixed full-admin Technical so switching map views no longer auto-opens a prior selected building panel and closing the Technical Assessment panel no longer reverts the map view to Engagement. Fixed full-admin Engagement floorplan controls so the Floorplan/Load buttons explicitly load the selected floor. Updated smoke checks and Hastings pre-release checklist to validate the redirects and full-admin Engagement/Technical views. Validation: `npm.cmd run smoke` passed and `npm.cmd run build` passed. Deployed to `main` as commit `952f421`; GitHub Pages workflow run `25115080995` completed successfully.
